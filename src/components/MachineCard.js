...
const MachineCard = ({ machine, updateStatus, state }) => {
  return (
    ...
    <div className="card mb-5 text-decoration-none">
      ...
      <div className="card-body">
        ...
        <p className="card-text">If necessary add credit to the card to use.</p>
      </div>
      <ul className="list-group list-group-flush">
        ...
        <li className="list-group-item">Status: {(state, machine.status)}</li>
      </ul>
      {machine.status === "free" && (
        <button
          type="button"
          className="btn btn-primary"
          onClick={() => updateStatus(machine.id)}
        >
          Start
        </button>
      )}
